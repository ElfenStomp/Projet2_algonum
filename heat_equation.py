...
##creation of matrix of the heat equation
def heat_matrix(N):
    m = N**2
    A = np.zeros([m, m])
    for i in range(0, m, 1):
        A[i][i] = -4        #diagonal
        if i % N != 0:   
            A[i][i - 1] = 1   #extra diagonal inferior
            A[i - 1][i] = 1   #extra diagonal superior
        if (i >= N):
            A[i - N][i] = 1   #diagonal superior
            A[i][i - N] = 1   #diagonal inferior
    return A
